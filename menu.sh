...
while true; do
  CHOICE=$(whiptail --title "BTC Node Installer" --menu "Choose an action" 22 82 14     1 "00 - Prerequisites"     2 "10 - Bitcoin Core"     3 "20 - LND"     4 "30 - Electrs"     5 "40 - ThunderHub"     6 "60 - Mempool"     7 "70 - Security hardening (Fail2ban/Monit/I2P/Tor DNS)"     8 "80 - Encrypted LND backups (systemd timer)"     9 "Show useful commands"     10 "Exit" 3>&1 1>&2 2>&3) || exit 0
  case "$CHOICE" in
    ...
    6) sudo ./scripts/60-mempool.sh ;;
    7) sudo ./scripts/70-security.sh ;;
    8) sudo ./scripts/80-backup.sh ;;
    9) whiptail --title "Useful commands" --msgbox "
Bitcoin: sudo -u bitcoin bitcoin-cli -datadir=/var/lib/bitcoind getblockchaininfo
LND:     lncli getinfo
Electrs: journalctl -u electrs -f
ThunderHub (Nginx): http://<host>:${NGINX_TH_PORT:-8088}
